  ...
  return (
    <>
      <div className="mb-3" >
        ...
        <button disabled={text.length===0} className="btn btn-primary mx-1 my-3" onClick={UpperClick}>
          Uppercase
        </button>
        <button disabled={text.length===0} className="btn btn-primary my-3 mx-1" onClick={Lowercase}>
          Lowercase
        </button>

        <button disabled={text.length===0} className="btn btn-primary my-3 mx-1" onClick={Capital}>
          Capitalize
        </button>
        <button disabled={text.length===0} className="btn btn-primary my-3 mx-1" onClick={Clear}>
          Clear
        </button>
      </div>
      ...
      <div className={`container my-3 border text-center text-${props.mode === 'light'?'dark':'light'}`}>
        ...
        <p className="fw-bold">
          ...
          <span className="fw-normal">
            {0.008 * text.split(/\s+/).filter((element)=>{return element.length!==0}).length} Min{" "}
          </span>
        </p>
      </div>
